 ...
 * @date 2017/1/13
  $images = new Thumb();
  $images->open('a.gif');
  $images->resize_to(100, 100, 'scale_fill');
  $images->add_text('1024i.com', 10, 20);
  $images->add_watermark('1024i.gif', 10, 50);
  $images->save_to('x.gif');
 */
...
class Thumb {
    ...
    // 添加水印图片
    public function add_watermark($path, $x = 0, $y = 0, $position = 9, $opacity = 1, $scale = 1) {
        $watermark = new \Imagick($path);
        $draw = new \ImagickDraw();
        $draw->setGravity(!$position ? $watermark::GRAVITY_SOUTHEAST : $position);
        $draw->setFillOpacity($opacity);
        $draw->composite($watermark->getImageCompose(), $x, $y, $watermark->getImageWidth() * $scale, $watermark->getimageheight() * $scale, $watermark);
    ...
    // 添加水印文字
    public function add_text($text, $x = 20, $y = 20, $angle = 0, $style = array(), $position = 9, $opacity = 1) {
        $draw = new \ImagickDraw();
        if (isset($style['font']))
    ...
    public function save_to($path, $filename = '') {
        ...
        $a = $this->image->getImageCompressionQuality() * 1;
        if ($a == 0) {
            $a = 60;
        }
...
//        }
        //add by liangpingzheng  date 2017/01/13 why 无法回传文件名称
        if ($this->type == 'gif') {
...
// 水平翻转
    ...
    /**
     * convert在线截图
     * @param $src_file 源文件名称
     * @param $target_file 新文件名称
     * @param int $maxW 宽
     * @param int $maxH 高
     * @param int $quality 质量
     * @return bool
     */
    public static function zoomConvertImage($src_file, $target_file, $maxW = 780, $maxH = 490, $quality = 100) {
        if (!file_exists($src_file) || $target_file == '') {
            return false;
        }
        ...
        if ($maxW == 0)
            $maxW = $srcW;
        //if($target_file == '') $target_file= $file_info['dirname'].'/'.$file_info['basename'];
        if ($srcW > $maxW || $srcH > $maxH) {
            if ($srcH > $maxH) {
                if ($srcW > $maxW) {
                    $dstH = ceil($srcH * ($maxW / $srcW));
                    $dstW = $maxW;
                    if ($dstH > $maxH) {
                        $dstH = $maxH;
            ...
            $thumnail = "{$dstW}x$dstH";
            $quality = " -quality " . $quality;
            $cmd = "convert " . $src_file . " -thumbnail $thumnail $quality  $target_file";
        } else {
            $cmd = "cp -f $src_file $target_file";
        }
        system($cmd);
        return $target_file;
    }
    ...
    // 保存到指定路径
    public function saveToSize($path) {
        //压缩图片质量
        $this->image->setimageformat('jpeg');
        $this->image->setimagecompression(imagick::compression_jpeg);
